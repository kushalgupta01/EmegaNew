...
document.addEventListener('DOMContentLoaded', async function() {
 ...
 //    }
	...
	// Check login
	{
		...
		if (page.userToken) {
			...
			if (response.ok && data.result == 'success') {
		        ...
		        let button4 = document.querySelector('#addOrders');
		        let supplier4 = window.page.user.supplier;
		        
		        if (button4.classList.contains(SUPPLIER_INFO[supplier4].id)) {
	...
	let lis = document.querySelectorAll('#menu-list li');

	for (let li of lis) {
		if (li.classList.contains(SUPPLIER_INFO[page.user.supplier].id)) {
		...
		for (let form of forms) {
			...
			if (form.type == 'stores') {
				...
				for (let id of storeIDs) {
					if (!SUPPLIER_INFO[page.user.supplier].stores.includes(id) && id != '30') continue;
					let store = stores[id];
					radioOptions.push({
						id: id,
						value: id,
	...
	let firstPanel = document.querySelector('#menu-list li:not(.hide)');
...
async function getOrders() {
	...
	do {
		let response, data;
		try {
			if (page.user.supplier==SUPPLIER['CombinedGroup']) {
				response = await fetch(apiServer+'orders/get/bigcommerce');
			} else if (page.user.supplier==SUPPLIER['HABITANIA']) {
				response = await fetch(apiServer+'orders/get/habitania');
			} else if (page.user.supplier==SUPPLIER['CATWALK']) {
				response = await fetch(apiServer+'orders/get/catwalk');
			} else if (page.user.supplier==SUPPLIER['RRV']) {
				response = await fetch(apiServer+'orders/get/rrv');
			} else if (page.user.supplier==SUPPLIER['ATPACK']) {
				response = await fetch(apiServer+'orders/get/atpack');
			} else if (page.user.supplier==SUPPLIER['PACOJAANSON']) {
				response = await fetch(apiServer+'orders/get/paco');
			} else {
...
function showPanel(data = {}) {
	...
	if (tabId == 'content-download') {
		...
		for (let input of page.els.downloadTrackingForm.querySelectorAll('input')) {
			let show = input.dataset.service == SERVICE_IDS['EBAY'] || input.value == 30 || input.dataset.service == SERVICE_IDS['WOOCOMMERCE'] || input.dataset.service == SERVICE_IDS['NETO'] || input.dataset.service == SERVICE_IDS['SHOPIFY']  || input.dataset.service == SERVICE_IDS['CATCH'];
		...
		if (data.result == 'success') {
		    ...
		    for (let order of newOrders) {
		    	...
		    	for (let item of items) {
		    		...
		    		aoa_row.push(item.sku || item.SKU || item.offer_sku);
		    		aoa_row.push(item.title || item.name || item.ProductName || item.product_title);
