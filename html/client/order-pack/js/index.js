...
window.userDetails = { username: null, password: null, userToken: '', type: null, supplier: null };
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.userDetails.type = localStorage.type;
    window.userDetails.supplier = localStorage.supplier;
    ...
    for (let store in stores) {
        ...
        if (!SUPPLIER_INFO[userDetails.supplier].stores.includes(store)) continue;       
...
function createPackingBtns() {
    ...
    for (let i = 0; i < packingTypes.length; i++) {
        ...
        btnMain.className = 'action-btn btn-darkcyan';
...
async function createOrderLeftTable() {
    ...
    for (let store in stores) {
        ...
        //th.textContent = SUPPLIER_INFO[window.userDetails.supplier].name;
