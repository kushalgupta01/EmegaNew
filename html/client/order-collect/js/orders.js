...
function showOrders(recordStore, recordNum) {
	var getItems = (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.STOCK || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.AWAITINGLIST || page.type == PAGE_TYPE.RTS);
...
async function showRecord(store, recordNum, getItems, appendEntry = false, combined = false) {
	...
	if (orderType == ORDER_TYPE.INTERNATIONAL && (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT) && rowData.BuyerCountry && countryCodes[rowData.BuyerCountry]) {
	...
	for (let i = 0; i < rowData.Items.length; i++) {
		...
		let itemQuantity = parseInt(isReplacement ? rowData.Items[i].ReplacedItem.quantity : rowData.Items[i].Quantity, 10);
		if ((page.user.type == USER_TYPE.ADMIN || page.user.type == USER_TYPE.USER || page.user.type == USER_TYPE.SUPPLIER || page.user.type == USER_TYPE.CLIENT) && getItems){
		...
		let tdData = [
			...
			[isReplacement ? rowData.Items[i].ReplacedItem.quantity : rowData.Items[i].Quantity, 'centre'],
		...
		if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.STOCK || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.AWAITINGLIST || page.type == PAGE_TYPE.RTS) {
		...
		if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED) {
			...
			if (page.tab == PAGE_TAB.HOBBYCO || page.tab == PAGE_TAB.B2B || page.tab == PAGE_TAB.HABITANIA || page.tab == PAGE_TAB.COMBINED 
				|| page.tab == PAGE_TAB.PACOJAANSON || page.tab == PAGE_TAB.ICTONE 
				|| page.tab == PAGE_TAB.CATWALK || page.tab == PAGE_TAB.ATPACK) {
				tableThBaySat.classList.add('hide');
				tdData.push(['<button class="item-report action-btn btn-grey">Report</button>'
				+ '<button id="downloadItem" class="item-download action-btn btn-dblue" data-type="downloaditem">Download</button>'
				+ '<button id="markparref" class="item-mark-partialrefund action-btn btn-dblue" data-type="markparref">Partial refund</button>'
				+ '<button id="markmodify" class="item-mark-modify action-btn btn-dblue" data-type="markmodify">Alternative</button>'
				...
				+ '<button id="editItem" class="item-edit action-btn btn-dblue" data-type="itemedit">Edit</button>'
				...
				tdData.push(['<button class="item-report action-btn btn-grey">Report</button>'
				+'<button id="add-inven" class="order-inventory action-btn btn-dblue">Add to Inventory</button>' 
				+ '<button id="markfp" class="item-mark-flatpack action-btn btn-dblue" data-type="markfp">Flatpack</button>'
				+ '<button id="markvr" class="item-mark-vr action-btn btn-dblue" data-type="markvr">VR</button>'
				...
				+ '<button id="markfactory" class="item-mark-factory action-btn btn-dblue" data-type="markfac">Factory</button>'
				+ '<button id="markcostco" class="item-mark-costco action-btn btn-dblue" data-type="markcos">Costco</button>'
				+ '<button id="downloadItem" class="item-download action-btn btn-dblue" data-type="downloaditem">Download</button>'
				+ '<button id="markadd" class="item-mark-add action-btn btn-dblue" data-type="markadd">Add</button>'
	...
	if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED) {
		...
		else if (page.tab == PAGE_TAB.MNB || page.tab == PAGE_TAB.VR || page.tab == PAGE_TAB.DO || page.tab == PAGE_TAB.FGB || page.tab == PAGE_TAB.MOR || page.tab == PAGE_TAB.JV || page.tab == PAGE_TAB.FACTORY  || page.tab == PAGE_TAB.HABITANIA || page.tab == PAGE_TAB.CATWALK) changeButtonStatus(recordBtns, '*');
		...
		//console.log(fgb);
		let FGBBtn = recordTemplate.querySelector('.' + pageName + ' #markfgb');
		if (fgb==1) {
...
function checkOrderCategory(store, recordNum) {
	...
	for (var cr_i = 0; cr_i < connectedRecords.length; cr_i++) {
		...
		for (var i = 0; i < connectedRowData.Items.length; i++) {
			...
			if (connectedRowDataToday.ME && page.type != PAGE_TYPE.AWAITINGLIST){
			...
			if (connectedRowDataToday.HABITANIA) {
				orderCategory[ORDER_DATA.HABITANIA] = true;	
			}
			if (connectedRowDataToday.CATWALK) {
			...
			if (SUPPLIER_INFO[SUPPLIER.HOBBYCO].stores.includes(store) && !connectedRowDataToday.WV && !connectedRowDataToday.ME  && store != 81) {
			...
			if (store == 81) {
				orderCategory[ORDER_DATA.B2B] = true;
			}
...
function getRecordOrderInfo(store, recordNum) {
	...
	if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED) {
		...
		if (rowDataToday.HABITANIA) {
			info[ORDER_DATA.HABITANIA] = true;
		}

		if (rowDataToday.CATWALK) {
		...
		if ((store == 1 || store == 6) && costco) {
			info[ORDER_DATA.COSTCO] = true;
		}

		if ((store == 1 || store == 6) && spwarehouse ) {
			info[ORDER_DATA.SPWAREHOUSE] = true;
		}

		if ((store == 1 || store == 6) && orbit ) {
			info[ORDER_DATA.ORBIT] = true;
		}

		if((store == 1 || store == 6) && wv) {
			info[ORDER_DATA.WV]  = true;
		}

		if((store == 1 || store == 6) && scholastic){
			info[ORDER_DATA.SCHOLASTIC] = true;
		}
		
		if((store == 1 || store == 6) && korimco){
			info[ORDER_DATA.KORIMCO] = true;
		}

		if((store == 1 || store == 6) && hyclor) {
			info[ORDER_DATA.HYCLOR] = true;
		}

		if((store == 1 || store == 6) && splosh) {
			info[ORDER_DATA.SPLOSH] = true;
		}

		if((store == 1 || store == 6 || store == 51) && sigma) {
			info[ORDER_DATA.SIGMA] = true;
		}

		if((store == 1 || store == 6) && misc) {
			info[ORDER_DATA.MISC] = true;
		}
		...
		if((store == 1 || store == 6 || store == 51) && kobayashi) {
			info[ORDER_DATA.KOBAYASHI] = true;
		}

		if((store == 1 || store == 6 || store == 51) && tprolls) {
			info[ORDER_DATA.TPROLLS] = true;
		}

		/*if (store == 81) {
			info[ORDER_DATA.B2B] = true;
		}*/
	...
	if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.AWAITINGLIST) {
		...
		if (SUPPLIER_INFO[SUPPLIER.HOBBYCO].stores.includes(store) && !rowDataToday.WV && !rowDataToday.ME && store != 81) {
	...
	else if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.AWAITINGLIST) {
		// Check for category symbol in each order item for all of the connected orders
