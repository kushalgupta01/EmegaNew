...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  if (localStorage.getItem('username') != 'waterwipes') $('#li-orderdownload').removeClass('hide');
	...
	if (page.userToken) {
        ...
        addListener('#record-refresh', 'click', function(e) {
...
async function getOrders() {
  do {
    ...
    try {
      response = await fetch(apiServer + "clients/stat/" + clientId);
  ...
  document.getElementById("total_collect_orders").textContent = data.total_collect_orders;
  document.getElementById("total_collected_orders").textContent = data.total_collected_orders;
  document.getElementById("total_sent_orders").textContent = data.total_sent_orders;
  document.getElementById("total_cost").textContent = '$' + (data.total_cost ? data.total_cost.toFixed(2) : '');
  document.getElementById("no_replacements").textContent = data.no_replacements;  

	loadOrdersToPanel("#panel-1", data.latest_orders);
	loadOrdersToPanel("#panel-2", data.latest_collect_orders);
  loadOrdersToPanel("#panel-3", data.latest_collected_orders);
