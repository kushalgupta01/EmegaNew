...
async function downloadInvoices() {
	...
	let receive_email = document.querySelector('#content-download-invoices #receive_email').value;

	if (!datefrom && !dateto && !receive_email) {
		page.notification.show('Please fill receive email.');
		return;
	}
	...
	var isShowSalesChannel = store==71 && document.getElementById("showSalesChannel").checked;
	...
	formData.append('receive_email', receive_email);
	
	
	let show_options = {
		showTotalCost: isShowTotalCost,
		showTotalPayment: isShowTotalPayment,
		showParcelWeight: isShowParcelWeight,
		showPostageCost: isShowPostageCost,
		showHanlingCost: isShowHanlingCost,
		showPaypalTransactionFee: isShowPaypalTransactionFee,
		showEbayFee: isShowEbayFee,
		showServiceFee: isShowServiceFee,
		showPaypalTotalFee: isShowPaypalTotalFee,
		showSalesChannel: isShowSalesChannel,
	};

	formData.append('show_options', JSON.stringify(show_options));
	...
	try {
		...
		if (data.result == 'success') {
			...
			if (receive_email) {
				page.notification.show('The invoices are downloading. You will receive the email contains download link when finished.', { background: 'bg-lgreen' });
...
async function showCostWeights(store) {
	...
	let saleschannelElement = document.getElementById("showSalesChannel");

	if (store==71) {
		saleschannelElement.checked = true;
		saleschannelElement.parentElement.style.visibility = '';
	} else {
		saleschannelElement.checked = false;
		saleschannelElement.parentElement.style.visibility = 'hidden';
	}
