...
async function generateLabelData(store, type, removeCombined = true, jsonOutput = false, repeat = 2, morelabel) {
	if (![ORDER_TYPE.AUSPOST, ORDER_TYPE.FASTWAY, ORDER_TYPE.EXPRESS, ORDER_TYPE.INTERNATIONAL, 
		ORDER_TYPE.FASTWAYFLATPACK, ORDER_TYPE.FASTWAYFLATPACK1KG, ORDER_TYPE.FASTWAYFLATPACK5KG].includes(type)) {
	...
	for (let rl_i = 0; rl_i < recordList.length; rl_i++) {
		...
		for (let co_i = 0; co_i < connectedRecords.length; co_i++) {
			...
			for (let it_i = 0; it_i < connectedRowData.Items.length; it_i++) {
				...
				// Quantity and weight
				let itemQuantity = recordItem.Quantity;
				let itemIndivQuantity = recordItem.Quantity;
				let itemWeight = 0;
				...
				if (itemEntries) {
					...
					for (let item of itemEntries) {
						if (item.num == recordItem.ItemNum) {
							...
							itemIndivQuantity = itemIndivQuantity * item.singleItemMultiple
				...
				itemData.IndivQuantity.push(itemIndivQuantity);
		...
		if (jsonOutput) {
			...
			if (type == ORDER_TYPE.AUSPOST || type == ORDER_TYPE.EXPRESS) {
				...
				let copyEntries = [
					...
					type == ORDER_TYPE.AUSPOST ? '7D83' : '7J83', // CHARGE CODE
				...
				let copyEntries = [
					...
					'', // MOBILE
					store == '4' ? (rowData.DeliveryNote ? rowData.DeliveryNote + ' Signature Required' : 'Signature Required') : (rowData.DeliveryNote || ''), // SPECIAL1
