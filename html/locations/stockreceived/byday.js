...
document.addEventListener('DOMContentLoaded', async function() {
	...
	await getStoreNameHasInventory();
	var isopen = false;
	var otherStores = document.querySelector('#invStores');
	otherStores.addEventListener('mouseup', () => {
		if (isopen) {
			...
			//console.log("changed to " + otherStores.value);
			isopen = false;
			searchStockReceived(otherStores.value);
		} 
		else {
			isopen = true;
		}
	});
	otherStores.addEventListener('blur', () => {
		isopen = false;
	});
...
async function searchStockReceived(store) {
	if (isNaN(store)==true) store=0;
	...
	let response = await fetch(apiServer+'stockreceived/byday/'+date+'/'+store, {method: 'post', headers: {'DC-Access-Token': page.userToken}});
	let data = await response.json();
...
async function getStoreNameHasInventory(){
	do {
		let response, data;
	try {
	    response = await fetch(apiServer + "getstorename/hasinventory", {headers: {'DC-Access-Token': page.userToken}});
	...
	let storeFilterSelect = document.querySelector('#invStores');
	for (let store of data.stores) {
		//console.log(store.store);
		if (['3','5','6','11','12','21','30','34'].includes(store.store)) continue;
		let option = document.createElement('option');
		option.value = store.store;
		option.textContent = stores[store.store].name;
...
/*async function searchStockReceived2(store) {
	document.getElementById('loading').style.display = 'block';
	let ths = document.querySelectorAll('.tr-header th');
	for (let th of ths ){
		if (th.innerHTML.startsWith("Location") || th.innerHTML.startsWith("Qty")){
