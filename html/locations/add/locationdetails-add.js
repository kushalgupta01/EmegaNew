...
function showBayInventory(){
	...
	let colsEditable = ['total Qty','itemBarcode'];
			...
			if (col == 'store') {
                ...
                let qty,id,type,store;
                    ...
                    td.addEventListener("blur", function(e){
                        ...
                        store = e.target.closest('tr').dataset.store;
                        ...
                        function myListener(event){
                            ...
                            // console.log('id'+id);
                            // console.log('qty'+qty);
                            updateQtyinBay(id,qty,type,store);
                        }
                ...
                td.addEventListener("blur", async function(e){
                    ...
                    let invID = e.target.parentNode.dataset.invID;
                    if (col == 'expiry') {
                    ...
                    if (!response.ok || responseData.result != 'success') {     
                        ...
                        swal('Barcode update successs.','','success');
                    }
...
async function updateQtyinBay(id,qty,type,store){
    let reason = document.querySelector('#reason').value;
    let formData = new FormData();
...
async function loadInventoryDetails(barcode,id) {
    ...
    if (itemBarcode === null || itemBarcode == ""){
        ...
        let parameter;
        let formData = new FormData();
        if (document.getElementById('itembarcode').checked == true){
            formData.append('barcode', barcode);
            parameter = 'Barcode';
        }
        else if (document.getElementById('itemno').checked == true){
            formData.append('itemno', barcode);
            parameter = 'Item No.';
        }
        ...
        else if (document.getElementById('itemcustomsku').checked == true){
            formData.append('customSku', barcode);
            parameter = 'Custom SKU';
        }
        else if (document.getElementById('itemname').checked == true){
            formData.append('itemName', barcode);
            parameter = 'Item Name containing'
        }
        ...
        if (response.ok && inventoryData.result == 'success') {
            ...
            if (updateList(barcode,id,parameter) == false) {
                // document.querySelector('#bay span').textContent = itemTBody.dataset.bay;
                ...
                for (let stock in inventorys) {
                    ...
                    tr.dataset.store = inventorys[stock].store;
        ...
        else if (inventoryData.result == 'Invalid parameter.'){
            page.notification.show('Invalid parameter.');
        }
        else {
            page.notification.show(parameter+': "'+barcode+'". Not found in Stock Inventory!');
...
function updateList(itemBarcode,id,parameter){
        let trs = document.querySelectorAll('#item-table-body tr');
        let sum = 0;
        let newqty = 1;
        for(let tr of trs) {
            if (/*itemBarcode == tr.querySelector('.itemBarcode').textContent &&*/ id == tr.dataset.invID){
                ...
                page.notification.show(parameter+': ' +itemBarcode+' +1 included.');
                sum++;
                highlight(tr);
            }
            else {};
...
async function checkInputRadioBox() {
    document.getElementById("content-inventory-searchfield").addEventListener("change", checkInputRadioBox);
    if (document.getElementById('itembarcode').checked == true) {
        document.getElementById('item-input-select').placeholder= 'Scan item barcode..';
    }
    else if (document.getElementById('itemno').checked == true){
        document.getElementById('item-input-select').placeholder= 'Load item number..';
    }
    else if (document.getElementById('itemsku').checked == true) {
        document.getElementById('item-input-select').placeholder= 'Load item sku..';
    }
    else if (document.getElementById('itemcustomsku').checked == true) {
        document.getElementById('item-input-select').placeholder= 'Load custom sku..';
    }
    else if (document.getElementById('itemname').checked == true) {
        document.getElementById('item-input-select').placeholder= 'Load item name..';
    }
}