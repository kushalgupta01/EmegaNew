...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById("load-bay").addEventListener("click", async(event) => {
            ...
            else if (checkLocat.startsWith('RS-') && document.getElementById('receiving-locations').checked == true){
                checkLocat = checkLocat.substr(3);
            }
            
            if (checkLocat === null || checkLocat == ""){
                    swal('Please select one location!','','warning');
                    return;
            }
            else{
                let location = checkLocat;
                if (document.getElementById('sts-newWarehouse').checked == true){
                    location = 'EMG-'+location;
                }
                else if (document.getElementById('receiving-locations').checked == true){
	 	...
	 	for (let col of cols) {
        	...
        	if (col == 'locations') {
                ...
                if (checkLocat.startsWith('emg-') && document.getElementById('sts-newWarehouse').checked == true){
                    ...
                    checkLocat = location.bay.substr(3);
                }
...
async function checkLocationTable() {
    ...
    if (document.getElementById('receiving-locations').checked == true) {
        let type = 7;
        await loadBayBulk(type);
        await bayTable();
    }
