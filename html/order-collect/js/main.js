...
document.addEventListener('DOMContentLoaded', async function() {
	...
	var titleContent = document.querySelector('.page-title');
	
	switch(pageTypeName){
		case "collect": 
			$('#co').addClass('active');
			titleContent.textContent="Collect Orders";
			break;
		case "partialcollect":
			$('#pco').addClass('active');
			titleContent.textContent="Partial Collect Orders";
			break;
		case "warehousecollect":
			$('#wco').addClass('active');
			titleContent.textContent="Warehouse Collect Orders";
			break;
		case "stock":
			$('#stk').addClass('active');
			$('#clientsBlock').css('opacity','0');
			titleContent.textContent="Out Of Stock Orders";
			break;
		case "ordered":
			$('#ord').addClass('active');
			titleContent.textContent="Ordered";
			break;
		case "b2b":
			$('#b2b').addClass('active');
			titleContent.textContent="Collected Orders";
			break;
		case "newOrders":
			$('#newo').addClass('active');
			titleContent.textContent="B2B Orders";
			break;
	...
	if (page.local) {
     ...
     var clients = document.getElementById('clientsBlock');
	 let others = document.createElement('select');
		others.setAttribute("id","clients");
	// Tabs
	...
	// newlabel. setAttribute("for","clients");
	// newlabel. innerHTML = "Clients: ";	
	// clients.appendChild(newlabel);
	...
	if ((page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT) && PAGE_TAB) {
		...
		if (page.user.supplier == SUPPLIER['NonSupplier']) {			
			...
			var option = document.createElement('option');
			option.textContent = 'Hobbyco';
			option.disabled = true;
			option.selected = 'selected';
			option.value = 'tab-hobbyco';
			others.appendChild(option);
			//newlabel.appendChild(others);
			// for (let tabID of tabIDs) {
			...
			for (let tabID of tabIDs) {
				if ([8,9,10,41,42,45,46,47].includes(tabID)) continue;
				var tab = PAGE_TAB_INFO[tabID];
				var option = document.createElement('option');
				option.id = tab.id;
				option.setAttribute('href', tab.href);
	...
	if (page.showBuckets && page.buckets && recordGroup) {
		var bukselect = document.createElement('select');
		//radio.type = 'radio';
		//radio.name = 'rg';
		// radio.id = 'rg';
		// label.setAttribute('for', 'rg');
		...
		for (let i = 1; i <= page.buckets; i++) {
			var bukoptions = document.createElement('option');
			
			bukoptions.id = 'rg'+i;
			bukoptions.value = i;
			bukoptions.textContent = i;
			bukselect.appendChild(bukoptions);
			
		}
         recordGroup.appendChild(bukselect);
		// Select the first bucket
		// recordGroup.querySelectorAll('input')[0].checked = true;
	}
	else if (recordGroup) {
	...
	addListener('#clientsBlock select', 'change', function(e) {
		e.preventDefault();	
		// Show tab
		let select = e.target;
	...
	// var el = document.getElementById('record-prev');
		// if(el){
		 // el.addEventListener('mouseup', showPrevRecord(), false);
	   // }

	document.getElementById('record-next').addEventListener('mouseup', function(e) {
		showNextRecord();
	});
	// var el1 = document.getElementById('record-next');
		// if(el1){
		 // el1.addEventListener('mouseup', function(e) {
				// showNextRecord();
			// });
	   // }
	...
	if ((page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.NEWORDERS || page.type == PAGE_TYPE.DEAL)) {
		document.getElementById('record-entries').addEventListener('click', function(e) {
			...
			//if (!target.classList.contains('cust-current-inventory')) return;
			let currentinventory = $(this).find('td.item-inventory a');
			if (!currentinventory[0].classList.contains('cust-current-inventory')) return;
