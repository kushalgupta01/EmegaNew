...
async function showOrders(recordStore, recordNum) {
	var getItems = (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.STOCK || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.AWAITINGLIST || page.type == PAGE_TYPE.RTS || page.type == PAGE_TYPE.NEWORDERS || page.type == PAGE_TYPE.DEAL);
	...
	for (var i = 0; i < connectedRecordsLen; i++) {
		...
		await showRecord(connectedRecords[i][0], connectedRecords[i][1], getItems, appendEntry, connectedRecordsCount > 1);
		appendEntry = true;
	}
...
async function showRecord(store, recordNum, getItems, appendEntry = false, combined = false) {
	...
	// sortItemsBylocation
    rowData.Items = await sortItemsByLocation(rowData.Items);
	...
	var recordTemplate = document.querySelector('#record-entry-template').content.cloneNode(true);	
	var headerContainer = document.querySelector('#record-header-template');
	...
	headerContainer.querySelector('.record-title .record-buyer-name').textContent = rowData.BuyerFullName;
	headerContainer.querySelector('.record-title .record-buyer-id').textContent = rowData.UserID;
	headerContainer.querySelector('.record-title .record-date').textContent = rowData.PaidDate;
	headerContainer.querySelector('.record-title .record-store').textContent = stores[store].name;
	headerContainer.querySelector('.record-title .record-num').textContent = rowData.SalesRecordID || rowData.RecordNum;
	headerContainer.querySelector('.record-title .record-db-id').textContent = rowData.DatabaseID;

	// Buyer country
	let recordBuyerCountry = headerContainer.querySelector('.record-title .record-buyer-country');
	if (orderType == ORDER_TYPE.INTERNATIONAL && (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.NEWORDERS) && rowData.BuyerCountry && countryCodes[rowData.BuyerCountry]) {
	...
	// Order status
	var recordOrderStatus = headerContainer.querySelector('.record-title .record-status');
	...
	postInternational.classList[(rowData.BuyerCountry.toUpperCase() == 'AU' || rowData.BuyerCountry.toUpperCase() == 'AUSTRALIA') ? 'add' : 'remove']('hide');
    ...
    // Hide all channel
	var channels = recordTemplate.querySelectorAll('.record-channel, .record-channel span');
	for (let i = 0; i < channels.length; i++) {
        channels[i].classList.add('hide');
    }
	    	...
	    	if (itemDetails[itemNum]) {
	    		...
	    		for (let itemData of itemDatas) {
	    			if (itemData.sku == itemSku) {
	    				foundItem = true;
	    				if (itemData.bundle && !item.ReplacedItem) {
	    					console.log(itemData.bundle);
	    					...
	    					for (let ite in bundle) {
	    						...
	    						console.log(JSON.parse(JSON.stringify(bundleItems[ite])));
	    						bundleData['Quantity'] = qty*parseInt(bundle[ite]);
	    						bundleData['SalePrice'] = price/num;
	    						bundleData['Parts'] = itemSku+' ' + i + '/'+count;
	    						if (item.AlterItem) bundleData['AlterItem'] = item.AlterItem;
	    						i++;
	...
	for (let i = 0; i < rowData.Items.length; i++) {
		if(rowData.Items[i].ReplacedItem) {
			isReplacement = true;
		}
	}
	if (isReplacement) {
		...
		for (let i = 0; i < rowData.Items.length; i++) {
			if(rowData.Items[i].ReplacedItem) {
				...
				let repalceItem = item.ReplacedItem;
				let newRepalceItem = {
					ItemNum: repalceItem.itemID,
					ItemTitle: repalceItem.name,
					LineItemID: item.LineItemID,
					Quantity: repalceItem.quantity,
					SKU: repalceItem.sku,
					SalePrice: repalceItem.price,
					TransID: null,
					VariationDetails: ""
				}
				newItems.push(newRepalceItem);
			}
		}
		rowData.Items = newItems;
	}
	for (let i = 0; i < rowData.Items.length; i++) {
		if(rowData.Items[i].scannedQty) {
			isScannedQty = true;
		}
	}
	for (let i = 0; i < rowData.Items.length; i++) {
		...
		/*if (isReplacement) {
			if (!rowData.Items[i].ReplacedItem) continue;
		}*/
		...
		if (page.tab == PAGE_TAB.PARTIALLYPICKED) {
			if (rowData.Items[i].scannedQty[0] != rowData.Items[i].scannedQty[1]) tr.classList.add('bg-partiallypicked');
		}
		...
		if (SUPPLIER_INFO[SUPPLIER.CombinedGroup].stores.includes(store) && VariationDetails) {
			// Save variation names/types and change item name
			tr.dataset.variationTypes = JSON.stringify(VariationDetails.map(x => x.display_name));
			itemTitleValue = rowData.Items[i].ItemTitle + '<br><strong>' + VariationDetails.map(x => (x.display_name || 'Variation')+':<span class="gap10"></span>'+x.display_value).join('<br>') + '</strong>';
		}
		...
		let tdData = [
			...
			[rowData.Items[i].SKU || '-', 'centre sku'],
			[itemTitleValue, 'title'],
			[parseFloat(rowData.Items[i].SalePrice).toFixed(2), null], // Price
			[subtotal.toFixed(2), null], // Subtotal
		];

		let tableThMissingQty = recordTemplate.querySelector('.record-items thead th[data-name="missingQty"]');
		...
		let tableThLocSelect = recordTemplate.querySelector('.record-items thead th[data-name="location-selected"]');

		if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.STOCK || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.AWAITINGLIST || page.type == PAGE_TYPE.RTS || page.type == PAGE_TYPE.NEWORDERS  || page.type == PAGE_TYPE.DEAL) {
			tableThMissingQty.classList.add('hide');
			...
			tableThLocSelect.classList.remove('hide');
			...
			tableThLocSelect.classList.add('hide');

			if(page.tab == PAGE_TAB.PARTIALLYPICKED){
				if (isScannedQty) {
					console.log(isScannedQty);

					tableThMissingQty.classList.remove('hide');
					let scannedQty = '';

					if(rowData.Items[i].scannedQty) {
						scannedQty = scannedQty + rowData.Items[i].scannedQty[1] - rowData.Items[i].scannedQty[0];
					}
					tdData.splice(1,0,[scannedQty, 'left']);
				}
			}
		...
		if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.NEWORDERS  || page.type == PAGE_TYPE.DEAL) {
			...
			if (page.tab == PAGE_TAB.HOBBYCO || page.tab == PAGE_TAB.QUARANTINE || /*page.tab == PAGE_TAB.B2BWholesale || page.tab == PAGE_TAB.B2BTransfer*/  page.tab == PAGE_TAB.B2BORDERS 
				|| page.tab == PAGE_TAB.WV || page.tab == PAGE_TAB.ME || page.tab == PAGE_TAB.LPG || page.tab == PAGE_TAB.HABITANIA 
				|| page.tab == PAGE_TAB.PACOJAANSON || page.tab == PAGE_TAB.ICTONE || page.tab == PAGE_TAB.HOBEXPRESS || page.tab == PAGE_TAB.SONSOFAMAZON 
				|| page.tab == PAGE_TAB.COMBINED || page.tab == PAGE_TAB.CATWALK || page.tab == PAGE_TAB.SPARTAN) {
				tableThBaySat.classList.add('hide');
				tdData.push(['<button class="item-report action-btn btn-grey btn btn-primary">Report</button>'
				+ '<button id="downloadItem" class="item-download action-btn btn btn-primary" data-type="downloaditem">Download</button>'
				+ '<button id="markparref" class="item-mark-partialrefund action-btn btn btn-primary" data-type="markparref">Partial Refund</button>'
				+ '<button id="markmodify" class="item-mark-modify action-btn btn btn-primary" data-type="markmodify">Alternative</button>'
				/*+ '<button id="markadd" class="item-mark-add action-btn" data-type="markadd">Add</button>'
				+ '<button id="deleteItem" class="item-delete action-btn" data-type="itemdelete">Delete</button>'*/
				+ '<button id="editItem" class="item-edit action-btn btn btn-primary" data-type="itemedit">Edit</button>'
				...
				tdData.push(['<button class="item-report action-btn btn-grey btn btn-primary">Report</button>'
				+'<button id="add-inven" class="order-inventory action-btn btn btn-primary">Add to Inventory</button>' 
				+ '<button id="markfp" class="item-mark-flatpack action-btn btn btn-primary" data-type="markfp">Flatpack</button>'
				+ '<button id="markvr" class="item-mark-vr action-btn btn btn-primary" data-type="markvr">VR</button>'
				...
				+ '<button id="markfactory" class="item-mark-factory action-btn btn btn-primary" data-type="markfac">Factory</button>'
				+ '<button id="markcostco" class="item-mark-costco action-btn btn btn-primary" data-type="markcos">Costco</button>'
				+ '<button id="downloadItem" class="item-download action-btn btn btn-primary" data-type="downloaditem">Download</button>'
				+ '<button id="markadd" class="item-mark-add action-btn btn btn-primary" data-type="markadd">Add</button>'
				...
				// + '<button id="marklpg" class="item-mark-lpg action-btn" data-type="marklpg">LPG</button>'
		...
		if(tdData?.length > 12){
			colSpanNumber = 4;			
		}else if (tdData?.length <= 6){
			colSpanNumber = 5;				
		}else if(tdData?.length > 8 && tdData?.length <= 11){
			colSpanNumber = 10;	
		}
		recordTemplate.querySelector('.footerSpanBlock').setAttribute('colSpan',colSpanNumber);		
		// Add row contents
	...
	recordTemplate.querySelector('.record-prices').innerHTML = '&nbsp;$'+round2(completeSubtotal)+'<br><hr>&nbsp;$'+round2(rowData.Postage)+'<br><hr>&nbsp;$'+round2(rowData.TotalPrice);
	...
	switch (orderType) {
		...
		case ORDER_TYPE.FASTWAYFLATPACK5KG:
			...
			showPostService = true;
			break;
		case ORDER_TYPE.DELIVERE:
			recordTemplate.querySelector('.record-postservice .delivere').classList.remove('hide');
			showPostService = true;
			break;
	}
	...
	var salesChannel = rowData.SalesChannel;
	switch (salesChannel) {
		case 'Website':
			recordTemplate.querySelector('.record-channel .website').classList.remove('hide');
			showChannel = true;
			break;
		case 'eBay':
			recordTemplate.querySelector('.record-channel .ebay').classList.remove('hide');
			showChannel = true;
			break;
		case 'Catch':
			recordTemplate.querySelector('.record-channel .catch').classList.remove('hide');
			showChannel = true;
			break;
		case 'Kogan':
			recordTemplate.querySelector('.record-channel .kogan').classList.remove('hide');
			showChannel = true;
			break;
		case 'Amazon':
			recordTemplate.querySelector('.record-channel .amazon').classList.remove('hide');
			showChannel = true;
			break;
	}

	//if (showChannel) recordTemplate.querySelector('.record-channel').classList.remove('hide');
	...
	if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED) {
		...
		else if (page.tab == PAGE_TAB.MNB || page.tab == PAGE_TAB.VR || page.tab == PAGE_TAB.DO || page.tab == PAGE_TAB.LPG || page.tab == PAGE_TAB.MOR || page.tab == PAGE_TAB.JV || page.tab == PAGE_TAB.FACTORY || page.tab == PAGE_TAB.HABITANIA || page.tab == PAGE_TAB.CATWALK) changeButtonStatus(recordBtns, '*');
		...
		for (let btn of buttons) {
			if (page.user.type == USER_TYPE['SUPPLIER']) {
				...
				if (btn.dataset.sup == 'true') {
					...
					if ((btn.classList.contains('order-ready-parcel')|| btn.classList.contains('order-notes')|| btn.classList.contains('order-select-all') ||
					btn.classList.contains('order-scan-bypass') || btn.dataset.status == 'cancelledoos' || btn.classList.contains('markAll-order-change-status') ||
					btn.dataset.status == 'partialcollect' || btn.classList.contains('item-mark-page') || btn.dataset.status == 'outofstock')){
					btn.disabled = false;
					btn.classList.remove('hide');
					}else{
					btn.disabled = true;
		...
		/*if (page.tab == PAGE_TAB.B2BWholesale || page.tab == PAGE_TAB.B2BTransfer) {
			let readytopackBtn = recordTemplate.querySelector('.order-ready-pack');
			if (readytopackBtn) readytopackBtn.classList.remove('hide');
		}*/

		let pageName = PAGE_INFO[page.type].name;
		let dailyorder = rowDataToday.DailyOrder;
		//console.log(dailyorder);
		let DoBtn = recordTemplate.querySelector('.' + pageName + ' #markdo');
		if (dailyorder==1) {
		...
		let lpg = rowDataToday.LPG;
		//console.log(lpg);
		let LPGBtn = recordTemplate.querySelector('.' + pageName + ' #marklpg');
		if (lpg==1) {
			LPGBtn.textContent = 'UnLPG';
		} else {
			LPGBtn.textContent = 'LPG Order';
		}

		let morlife = rowDataToday.MORLIFE;
		//console.log(morlife);
		let MorlifeBtn = recordTemplate.querySelector('.' + pageName + ' #markmor');
		if (morlife==1) {
		...
		let SPWBtn = recordTemplate.querySelector('.' + pageName + ' #markspw');
		if (spwarehouse==1) {
		...
		//console.log(orbit);
		let OrbBtn = recordTemplate.querySelector('.' + pageName + ' #markorb');
		if (orbit==1) {
		...
		//console.log(wv);
		let WVBtn = recordTemplate.querySelector('.' + pageName + ' #markwv');
		if (wv==1) {
		...
		let me = rowDataToday.ME;
		//console.log(wv);
		let MEBtn = recordTemplate.querySelector('.' + pageName + ' #markme');
		if (me==1) {
			MEBtn.textContent = 'UnME';
		} else {
		...
		let SchoBtn = recordTemplate.querySelector('.' + pageName + ' #markscho');
		if (scholastic==1) {
		...
		//console.log(korimco);
		let KorBtn = recordTemplate.querySelector('.' + pageName + ' #markkor');
		if (korimco==1) {
		...
		//console.log(hyclor);
		let HycBtn = recordTemplate.querySelector('.' + pageName + ' #markhyc');
		if (hyclor==1) {
		...
		//console.log(splosh);
		let SploBtn = recordTemplate.querySelector('.' + pageName + ' #marksplo');
		if (splosh==1) {
		...
		//console.log(sigma);
		let SigBtn = recordTemplate.querySelector('.' + pageName + ' #marksig');
		if (sigma==1) {
		...
		//console.log(sigma);
		let MiscBtn = recordTemplate.querySelector('.' + pageName + ' .item-mark-misc');
		if (misc==1) {
		...
		let jv = rowDataToday.JV;
		//console.log(jv);
		let jvBtn = recordTemplate.querySelector('.' + pageName + ' #markjv');
		if (jv==1) {
			jvBtn.textContent = 'UnJV';
		} else {
			jvBtn.textContent = 'JV';
		}

		let factory = rowDataToday.FACTORY;
		//console.log(factory);
		let FacBtn = recordTemplate.querySelector('.' + pageName + ' #markfac');
		...
		// //console.log(sixpack);
		// let sixpBtn = recordTemplate.querySelector('.' + pageName + ' #marksix');
		// if (sixpack==1) {
		...
		// //console.log(tenpack);
		// let tenpBtn = recordTemplate.querySelector('.' + pageName + ' #markten');
		// if (tenpack==1) {
		...
		// let twentypBtn = recordTemplate.querySelector('.' + pageName + ' #marktwenty');
		// if (twentypack==1) {
		...
		// let thirtypBtn = recordTemplate.querySelector('.' + pageName + ' #markthirty');
		// if (thirtypack==1) {
		...
		// let sixtypBtn = recordTemplate.querySelector('.' + pageName + ' #marksixty');
		// if (sixtypack==1) {
		...
		// //console.log(gucci);
		// let gucciBtn = recordTemplate.querySelector('.' + pageName + ' #markgucci');
		// if (gucci==1) {
		...
		let kobBtn = recordTemplate.querySelector('.' + pageName + ' #markkob');
		if (kobayashi==1) {
		...
		let tprBtn = recordTemplate.querySelector('.' + pageName + ' #marktprolls');
		if (tprolls==1) {
	...
	if (page.type == PAGE_TYPE.B2B) {
		if (page.tab == PAGE_TAB.PARTIALLYPICKED) {
			recordTemplate.querySelector('.order-mark-fullypicked').classList.remove('hide');
			recordTemplate.querySelector('.b2b .notice').classList.remove('hide');
			recordTemplate.querySelector('.order-mark-dispatch').classList.remove('hide');
		} 
		else if (page.tab == PAGE_TAB.FULLYPICKED) {
			recordTemplate.querySelector('.order-ready-to-print').classList.remove('hide');
			recordTemplate.querySelector('.order-mark-pack').classList.remove('hide');
		}
	}

	if (page.type == PAGE_TYPE.REFUNDS) {
		if (page.tab == PAGE_TAB.PARTIALREFUND) {
			recordTemplate.querySelector('.order-partial-refund-done').classList.remove('hide');
		} 
		else {
			recordTemplate.querySelector('.order-partial-refund-done').classList.add('hide');
		}	
	}
...
function changeButtonStatus(recordBtns, orderType) {
	var orderTypeParcel = [ORDER_TYPE.AUSPOST, ORDER_TYPE.FASTWAY, ORDER_TYPE.FLATPACK, ORDER_TYPE.PARCEL, ORDER_TYPE.FASTWAYFLATPACK, ORDER_TYPE.FASTWAYFLATPACK1KG, ORDER_TYPE.FASTWAYFLATPACK5KG, ORDER_TYPE.DELIVERE];
...
function checkOrderCategory(store, recordNum) {
	...
	for (var cr_i = 0; cr_i < connectedRecords.length; cr_i++) {
		...
		for (var i = 0; i < connectedRowData.Items.length; i++) {
			...
			if (connectedRowDataToday.LPG && page.type != PAGE_TYPE.AWAITINGLIST && (!connectedRowData.PostService || !connectedRowData.PostService.toLowerCase().includes('express'))) {
				orderCategory[ORDER_DATA.LPG] = true;
			} 
			...
			if (connectedRowDataToday.WV && page.type != PAGE_TYPE.AWAITINGLIST && (!connectedRowData.PostService || !connectedRowData.PostService.toLowerCase().includes('express'))){
				orderCategory[ORDER_DATA.WV] = true;
			}

			if (connectedRowDataToday.ME && page.type != PAGE_TYPE.AWAITINGLIST && (!connectedRowData.PostService || !connectedRowData.PostService.toLowerCase().includes('express'))){
				orderCategory[ORDER_DATA.ME] = true;
			}
			...
			if (connectedRowDataToday.JV) {
				orderCategory[ORDER_DATA.JV] = true;
			}
			...
			if (SUPPLIER_INFO[SUPPLIER.HOBBYCO].stores.includes(store) && store != 81 && store != 82) {
				if (connectedRowData.PostService && connectedRowData.PostService.toLowerCase().includes('express') && connectedRowData.CouponCode != "IMBORED") {
					orderCategory[ORDER_DATA.HOBEXPRESS] = true;
				} else if (!connectedRowDataToday.WV && !connectedRowDataToday.ME && !connectedRowDataToday.LPG) {
					if (connectedRowData.CouponCode == "IMBORED") {
						orderCategory[ORDER_DATA.QUARANTINE] = true;
					} else {
			...
			// if (store == 81) {
			// 	orderCategory[ORDER_DATA.B2BWholesale] = true;
			// }
			// if (store == 82) {
			// 	orderCategory[ORDER_DATA.B2BTransfer] = true;
			// }

			if (store == 81 || store == 82) {
				orderCategory[ORDER_DATA.B2BORDERS] = true;
			}

			if (connectedRowDataToday.OrderStatus == 21 && page.type == PAGE_TYPE.B2B) {
				orderCategory[ORDER_DATA.PARTIALLYPICKED] = true;
				continue;
			}
			if (connectedRowDataToday.OrderStatus == 22 && page.type == PAGE_TYPE.B2B) {
				orderCategory[ORDER_DATA.FULLYPICKED] = true;
				continue;
			}
			if (store == 61 && page.type != PAGE_TYPE.AWAITINGLIST) {
				orderCategory[ORDER_DATA.CHARLICHAIR] = true;
			}
			if (SUPPLIER_INFO[SUPPLIER.CombinedGroup].stores.includes(store) && page.type != PAGE_TYPE.AWAITINGLIST) {
			...
			if (SUPPLIER_INFO[SUPPLIER.HABITANIA].stores.includes(store)) {
				orderCategory[ORDER_DATA.HABITANIA] = true;	
			}
			if (SUPPLIER_INFO[SUPPLIER.CATWALK].stores.includes(store)) {
				orderCategory[ORDER_DATA.CATWALK] = true;
			}
			if (SUPPLIER_INFO[SUPPLIER.SonsOfAmazon].stores.includes(store)) {
				orderCategory[ORDER_DATA.SONSOFAMAZON] = true;
			}
			if (SUPPLIER_INFO[SUPPLIER.AUTOWELL].stores.includes(store)) {
				orderCategory[ORDER_DATA.AUTOWELL] = true;
			} 
			if (SUPPLIER_INFO[SUPPLIER.TRINITYCONNECT].stores.includes(store)) {
				orderCategory[ORDER_DATA.TRINITYCONNECT] = true;
			} 
			/*if (store == 4) {
				orderCategory[ORDER_DATA.JV] = true;
			}*/
			if (store == 102) {
				orderCategory[ORDER_DATA.CIRCULAR2NDS] = true;
			}
			if (store == 104) {
				orderCategory[ORDER_DATA.ELIABATHROOMS] = true;
			} 
			if (store == 105) {
				orderCategory[ORDER_DATA.SPARTAN] = true;
			} 
			...
			if (item.SKU && item.SKU.startsWith('RRV_') && store==1) {
				orderCategory[ORDER_DATA.RRV] = true;
			}

			if (item.SKU && item.SKU.toLowerCase().startsWith('swd') && store==1) {
				orderCategory[ORDER_DATA.SWD] = true;
			}

			if (item.SKU && item.SKU.startsWith('HPD_') && store==1) {
				orderCategory[ORDER_DATA.HPD] = true;
			}

			if (item.SKU && item.SKU.startsWith('deal') && store==1) {
				orderCategory[ORDER_DATA.DEAL] = true;
			}

			if (item.SKU && item.SKU=="deal_HUGGIESBABYWIPES" && store==1) {
				orderCategory[ORDER_DATA.HUGGIES] = true;
			}

			if (item.SKU && item.SKU=="deal_FINISH_LEMON_110PACK" && store==1) {
				orderCategory[ORDER_DATA.FINISH] = true;
			}

			if (item.SKU && item.SKU=="deal_ORALB_ELECTRIC_MEDIUM" && store==1) {
				orderCategory[ORDER_DATA.ORALB] = true;
			}

			if (store == 106) {
				orderCategory[ORDER_DATA.ATPACK] = true;
			}

			/*if (store == 71 && connectedRowData.CouponCode == "IMBORED") {
				orderCategory[ORDER_DATA.QUARANTINE] = true;
			}*/

			if (item.SKU && item.SKU.startsWith('PJ_') && store==1) {
				orderCategory[ORDER_DATA.PACOJAANSON] = true;
			}
...
function getRecordOrderInfo(store, recordNum) {
	...
	var habitania = false;
	var catwalk = false;
	var partiallypicked = false;
	var fullypicked = false;
	var rrv = false;
	var swd = false;
	var hpd = false;
	var deal = false;
	...
	if (rowDataToday.OrderStatus == 21) {
		partiallypicked = true;
	}

	if (rowDataToday.OrderStatus == 22) {
		fullypicked = true;
	}
	...
	for (let cr_i = 0; cr_i < connectedRecords.length; cr_i++) {
		...
		if (connectedRecord[0]=="71" && connectedRowData.CouponCode == "IMBORED") {
			quarantine = true;
		}

		//if (recordNum==68717) console.log(connectedRowData.Items);
		for (let connectedRowItem of connectedRowData.Items) {
			if (connectedRowItem.SKU && connectedRowItem.SKU.startsWith('RRV_')) {
				rrv = true;
			}
			if (connectedRowItem.SKU && connectedRowItem.SKU.toLowerCase().startsWith('swd')) {
				swd = true;
			}
			if (connectedRowItem.SKU && connectedRowItem.SKU.startsWith('HPD_')) {
				hpd = true;
			}
			if (connectedRowItem.SKU && connectedRowItem.SKU.startsWith('deal')) {
				deal = true;
			}
			if (connectedRowItem.SKU && connectedRowItem.SKU=="deal_HUGGIESBABYWIPES") {
				huggies = true;
			}
			if (connectedRowItem.SKU && connectedRowItem.SKU=="deal_FINISH_LEMON_110PACK") {
				finish = true;
			}
			if (connectedRowItem.SKU && connectedRowItem.SKU=="deal_ORALB_ELECTRIC_MEDIUM") {
				oralb = true;
			}
			if (connectedRowItem.SKU && connectedRowItem.SKU.startsWith('PJ_')) {
				pacojaanson = true;
			}
	...
	if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.NEWORDERS || page.type == PAGE_TYPE.DEAL) {
		...
		if (rowDataToday.DailyOrder) {
			info[ORDER_DATA.DO] = true;
		}

		if (rowDataToday.LPG && info.OrderType != ORDER_TYPE.EXPRESS) {
			info[ORDER_DATA.LPG] = true;
		}

		if (rowDataToday.MORLIFE) {
			info[ORDER_DATA.MORLIFE] = true;
		}

		if (rowDataToday.SPWAREHOUSE) {
			info[ORDER_DATA.SPWAREHOUSE] = true;
		}

		if (rowDataToday.ORBIT) {
			info[ORDER_DATA.ORBIT] = true;
		}

		if (rowDataToday.WV && info.OrderType != ORDER_TYPE.EXPRESS) {
			info[ORDER_DATA.WV] = true;
		}

		if (rowDataToday.ME && info.OrderType != ORDER_TYPE.EXPRESS) {
			info[ORDER_DATA.ME] = true;
		}

		if(rowDataToday.SCHOLASTIC) {
			info[ORDER_DATA.SCHOLASTIC] = true;
		}

		if(rowDataToday.KORIMCO) {
			info[ORDER_DATA.KORIMCO] = true;
		}

		if(rowDataToday.HYCLOR) {
			info[ORDER_DATA.HYCLOR] = true;
		}

		if(rowDataToday.SPLOSH) {
			info[ORDER_DATA.SPLOSH] = true;
		}

		if(rowDataToday.SIGMA) {
			info[ORDER_DATA.SIGMA] = true;
		}

		if(rowDataToday.MISC) {
			info[ORDER_DATA.MISC] = true;
		}

		// if(rowDataToday.SIXPACK) {
		// 	info[ORDER_DATA.SIXPACK] = true;
		// }

		// if(rowDataToday.TENPACK) {
		// 	info[ORDER_DATA.TENPACK] = true;
		// }

		// if(rowDataToday.TWENTYPACK) {
		// 	info[ORDER_DATA.TWENTYPACK] = true;
		// }

		// if(rowDataToday.THIRTYPACK) {
		// 	info[ORDER_DATA.THIRTYPACK] = true;
		// }

		// if(rowDataToday.SIXTYPACK) {
		// 	info[ORDER_DATA.SIXTYPACK] = true;
		// }

		// if(rowDataToday.GUCCI) {
		// 	info[ORDER_DATA.GUCCI] = true;
		// }

		if(rowDataToday.KOBAYASHI) {
			info[ORDER_DATA.KOBAYASHI] = true;
		}

		if (rowDataToday.TPROLLS) {
			info[ORDER_DATA.TPROLLS] = true;
		}

		if (store == 11 || store == 12) {
			// MNB
		...
		if (SUPPLIER_INFO[SUPPLIER.CombinedGroup].stores.includes(store)) {
		...
		if (SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && costco) {
			info[ORDER_DATA.COSTCO] = true;
		}

		if (SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && spwarehouse ) {
			info[ORDER_DATA.SPWAREHOUSE] = true;
		}

		if (SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && orbit ) {
			info[ORDER_DATA.ORBIT] = true;
		}

		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && wv) {
			info[ORDER_DATA.WV]  = true;
		}

		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && scholastic){
			info[ORDER_DATA.SCHOLASTIC] = true;
		}
		
		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && korimco){
			info[ORDER_DATA.KORIMCO] = true;
		}

		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && hyclor) {
			info[ORDER_DATA.HYCLOR] = true;
		}

		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && splosh) {
			info[ORDER_DATA.SPLOSH] = true;
		}

		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && sigma) {
			info[ORDER_DATA.SIGMA] = true;
		}

		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && misc) {
			info[ORDER_DATA.MISC] = true;
		}

		// if((store == 1 || store == 6 || store == 51) && sixpack) {
		// 	info[ORDER_DATA.SIXPACK] = true;
		// }

		// if((store == 1 || store == 6 || store == 51) && tenpack) {
		// 	info[ORDER_DATA.TENPACK] = true;
		// }

		// if((store == 1 || store == 6 || store == 51) && twentypack) {
		// 	info[ORDER_DATA.TWENTYPACK] = true;
		// }

		// if((store == 1 || store == 6 || store == 51) && thirtypack) {
		// 	info[ORDER_DATA.THIRTYPACK] = true;
		// }

		// if((store == 1 || store == 6 || store == 51) && sixtypack) {
		// 	info[ORDER_DATA.SIXTYPACK] = true;
		// }

		// if((store == 1 || store == 6 || store == 51) && gucci) {
		// 	info[ORDER_DATA.GUCCI] = true;
		// }

		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && kobayashi) {
			info[ORDER_DATA.KOBAYASHI] = true;
		}

		if(SUPPLIER_INFO[SUPPLIER.EMEGA].stores.includes(store) && tprolls) {
			info[ORDER_DATA.TPROLLS] = true;
		}

		// if (store == 81) {
		// 	info[ORDER_DATA.B2BWholesale] = true;
		// }	

		// if (store == 82) {
		// 	info[ORDER_DATA.B2BTransfer] = true;
		// }

		if (store == 81 || store == 82) {
			info[ORDER_DATA.B2BORDERS] = true;
		}	

		if (SUPPLIER_INFO[SUPPLIER.CATWALK].stores.includes(store)) {
			info[ORDER_DATA.CATWALK] = true;
		}

		if (SUPPLIER_INFO[SUPPLIER.SonsOfAmazon].stores.includes(store)) {
			info[ORDER_DATA.SONSOFAMAZON] = true;
		}

		if (SUPPLIER_INFO[SUPPLIER.AUTOWELL].stores.includes(store)) {
			info[ORDER_DATA.AUTOWELL] = true;
		}

		if (SUPPLIER_INFO[SUPPLIER.TRINITYCONNECT].stores.includes(store)) {
			info[ORDER_DATA.TRINITYCONNECT] = true;
		}

		if (store == 102) {
			info[ORDER_DATA.CIRCULAR2NDS] = true;
		}

		if (store == 104) {
			info[ORDER_DATA.ELIABATHROOMS] = true;
		}

		if (store == 105) {
			info[ORDER_DATA.SPARTAN] = true;
		}

		if (store == 1 && rrv) {
			info[ORDER_DATA.RRV] = true;
		}

		if (store == 1 && swd) {
			info[ORDER_DATA.SWD] = true;
		}

		if (store == 1 && hpd) {
			info[ORDER_DATA.HPD] = true;
		}

		if (store == 1 && deal) {
			info[ORDER_DATA.DEAL] = true;
		}

		if (store == 1 && huggies) {
			info[ORDER_DATA.HUGGIES] = true;
		}
		if (store == 1 && finish) {
			info[ORDER_DATA.FINISH] = true;
		}
		if (store == 1 && oralb) {
			info[ORDER_DATA.ORALB] = true;
		}

		if (store == 106) {
			info[ORDER_DATA.ATPACK] = true;
		}

		/*if (store == 71 && quarantine) {
			info[ORDER_DATA.QUARANTINE] = true;
		}*/

		if (store == 1 && pacojaanson) {
			info[ORDER_DATA.PACOJAANSON] = true;
		}

	}

		
	if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.AWAITINGLIST) {
		if(rowDataToday.JV) {
			info[ORDER_DATA.JV] = true;
		}

		if(rowDataToday.FACTORY) {
			info[ORDER_DATA.FACTORY] = true;
		}

			
		if (SUPPLIER_INFO[SUPPLIER.HOBBYCO].stores.includes(store) && store != 81 && store != 82) {
			if (info.OrderType==ORDER_TYPE.EXPRESS && !quarantine) {
				info[ORDER_DATA.HOBEXPRESS] = true;
			} else if (!rowDataToday.WV && !rowDataToday.ME && !rowDataToday.LPG) {
				if (quarantine) {
					info[ORDER_DATA.QUARANTINE] = true;
				} else {
		...
		/*if((store == 1 || store == 4 || store == 51) && jv) {
			info[ORDER_DATA.JV] = true;
		}

		if((store == 1 || store == 4 || store == 51) && factory) {
			info[ORDER_DATA.FACTORY] = true;
		}*/

		if (SUPPLIER_INFO[SUPPLIER.HABITANIA].stores.includes(store)) {
			info[ORDER_DATA.HABITANIA] = true;
		}
	}
	...
	if (page.type == PAGE_TYPE.B2B && partiallypicked){
		info[ORDER_DATA.PARTIALLYPICKED] = true;
	}
	if (page.type == PAGE_TYPE.B2B && fullypicked){
		info[ORDER_DATA.FULLYPICKED] = true;
	}
	...
	else if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.AWAITINGLIST || page.type == PAGE_TYPE.NEWORDERS) {
		// Check for category symbol in each order item for all of the connected orders
	...
	if (info.OrderType === null) {
		if (flatpacked || info.OrderType == ORDER_TYPE.FLATPACK) {
			...
			if (fastway) {
				...
				/*if (delivereCodes.includes(rowData.BuyerPostCode) 
				    && (stores[store].service != SERVICE_IDS.EBAY || (store==71 && rowData.SalesChannel != 'eBay' && rowData.SalesChannel != 'Website'))) {
					info.OrderType = ORDER_TYPE.DELIVERE;
				}*/
				if (fwfp && (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT)) {
					if (fwfp == '0.3') {
						info.OrderType = ORDER_TYPE.FASTWAYFLATPACK;
					} else if (fwfp == '1') {
			...
			if (store==71 && rowData.SalesChannel=='Website') {
				info.OrderType = ORDER_TYPE.AUSPOST;
			}
		}
	}
...
function getRecordsOfType(storeID, type = null, sort = false, connectedRecordsTogether = false, morelabel) {
	...
	var recordList = [];
	var combinedGroupStores = SUPPLIER_INFO[SUPPLIER.CombinedGroup].stores;
...
async function sortItemsByLocation(items) {
	var newItems = [];
	...
	for (var i = 0; i < items.length; i++) {
		...
		skuList.push({barcode:null,sku:null,customSku:items[i].SKU});

	}

	await getInventoryDetails(skuList); 

	let locationList = [];
	for (let item of items) {
		let sku = item.SKU;
		if (inventoryDetails[sku]) {
			if (inventoryDetails[sku][0].locations.length < 1) continue;
			let locations = inventoryDetails[sku][0].locations;
			for (let loc of locations) {
				if (loc.id != 'qvb' && loc.id != 'pick' && loc.id != 'bulk' && loc.indivQty > 0) {
					if (loc.bay && loc.bay.startsWith('EMG-')) {
						locationList.push(loc.bay);
						item['choosenBay'] = loc.bay;
						break;
					}
	...
	/*locationList.sort(function(a,b){
	    return a.substring(3, 6) - b.substring(3, 6);
	});
	...
	for (let loc of locationList) {
		for (var i = 0; i< items.length; i++) {
			if (items[i] === undefined) continue;
			if (items[i].choosenBay == loc) {
				delete items[i].choosenBay;
