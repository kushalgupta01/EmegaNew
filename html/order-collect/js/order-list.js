...
async function createRecordList() {
	...
	for (let store in stores) {
		...
		for (let recordNum of recordNums) {
			...
			if (rowDataToday.constructor == Object) {
				...
				else if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT || page.type == PAGE_TYPE.WAREHOUSECOLLECT || page.type == PAGE_TYPE.AWAITINGLIST || page.type == PAGE_TYPE.RTS || page.type == PAGE_TYPE.ORDERED || page.type == PAGE_TYPE.REFUNDS || page.type == PAGE_TYPE.B2B || page.type == PAGE_TYPE.NEWORDERS) {
			...
			// Save record
			recordList.push([store, recordNum,rowDataToday.GroupID]);
		}
	}
	...
	for (let i = 0; i < recordList.length; i++) {
		let store = recordList[i][0], recordNum = recordList[i][1], GroupID = recordList[i][2];
		...
		if (page.type == PAGE_TYPE.COLLECT || page.type == PAGE_TYPE.PARTIALCOLLECT) {
		...
		if (page.type == PAGE_TYPE.NEWORDERS) {
			/*let input = document.createElement('input');
			input.type = 'text';
			...
			*/
			...
			select.style.padding = '0px';
		        for (let act=1 ; act<=40; act++){
		        	let option = document.createElement('option');
		          	option.value = act;
		          	option.textContent = act;
		          	select.appendChild(option);
		        }
		        select.value = parseInt(GroupID);
...
async function saveSelectedCage(e){
	let li = e.target.closest('li');
	let recordID = li.dataset.record;
	let group = li.querySelector('select').value;

	let cagesSelected = [];
	let lis = document.querySelectorAll('#record-list>ul>li:not([class="disabled"])');
	for (let l of lis){
		if (l.dataset.groupId != 'null' && cagesSelected.includes(l.dataset.groupId) === false) cagesSelected.push(l.dataset.groupId);
	}
	if (cagesSelected.includes(group)){
		alert('Cage already in use.');
		if (li.dataset.groupId == 'null'){
			li.querySelector('select').value = 0;
		}
		else {
			li.querySelector('select').value = li.dataset.groupId;
		}
		return;
	}
	...
	if (response.ok && result.result == 'success') { 
		page.notification.show('Cage number updated.');
		li.dataset.groupId = li.querySelector('select').value
		saleRecords[li.dataset.store].today[li.dataset.record].GroupID = li.dataset.groupId;
		if (document.querySelector('#record-container>#record-main>#record-entries>.record-entry').dataset.recordNum == li.dataset.record){
			var recordGroup = document.getElementById('record-group');
			recordGroup.querySelector('input[name="rg"][value="'+li.dataset.groupId+'"]').checked = true;
		}
	}
	else {
		page.notification.show(result.result);
	}
}
