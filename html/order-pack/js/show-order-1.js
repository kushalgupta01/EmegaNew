...
async function showOrder(orderDetails, tracking) {
    ...
    showTracking();
    document.querySelector('.record-trackingid span').textContent = tracking ? JSON.parse(tracking).slice(-1) : '';
    if (pageType != 3) {
...
function showItems(orderDetails, tableBody) {
    ...
    for (var i = 0; i < orderDetails.Items.length; i++) {
        ...
        var tdData = [
            ...
            calcTtbs(orderDetails.Items[i].Quantity,
                orderDetails.Items[i].multiple) || null, // To be scanned
        ...
        for (var td_i = 0; td_i < tdData.length; td_i++) {
            ...
            } else if (td_i == 5){ // Everything else (image)
                if (tdData[td_i]) {
