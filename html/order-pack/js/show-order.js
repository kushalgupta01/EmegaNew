...
async function showOrder(orderDetails, tracking) {
    ...
    var trackingNumbers = tracking ? JSON.parse(tracking) : [];

    let existTrackings = {};
    for (let pws of orderDetails.parcelWeights) {
        for (let pw of pws) {
            if (!existTrackings[pw.trackingNumber]) {
                existTrackings[pw.trackingNumber] = [pw.type, pw.weight]
            }
        }
    }
    ...
    for (let track of trackingNumbers) {
        if (existTrackings[track]) {
            trackingData.push([track].concat(existTrackings[track]));
        } else {
            trackingData.push([track].concat(['0','']));
        }
    }
    
    if (trackingData.length) {
        $(".save-weight").addClass('hide');
        var html = [];

        for (let trackingNumber of trackingData) {
            html.push([
                '<tr>',
                    '<td>' + trackingNumber[0].trim() + '</td>',
                    '<td class="hide">' + trackingNumber[1] + '</td>',
                    '<td><input type="number" class="parcelWeight" value="'+trackingNumber[2]+'"/></td>',
                '</tr>',
            ]);
        }

        $("#save-parcelweights").removeClass('hide');
        $("#tblParcelWeights tbody").html(html.join(''));

    } else {
        $(".save-weight").removeClass('hide');

        $("#save-parcelweights").addClass('hide');
        $("#tblParcelWeights tbody").html('');
    }
...
function showItems(orderDetails, tableBody) {
    ...
    for (var i = 0; i < orderDetails.Items.length; i++) {
        ...
        var barcodeCheck = checkItemBarcode(orderDetails.Items[i].barcode);
        var singleBarcodeCheck = checkItemBarcode(orderDetails.Items[i].singleItemBarcode);
        var tdData = [
        ...
        tr.dataset.orderid = orderDetails.Items[i].orderID || null;
        tr.dataset.lineitemid = orderDetails.Items[i].LineItemID || null;
        // If no item barcode, assume this particular item is scanned
        if (barcodeCheck || singleBarcodeCheck) {
        ...
        for (var td_i = 0; td_i < tdData.length; td_i++) {
            ...
            if (td_i == 5) { // To be scanned cell
                ...
                if (tdData[td_i] != null && singleBarcodeCheck) {
                    var currentSpan = document.createElement('span');
                    var totalSpan = document.createElement('span');
...
function prepareImage(imageUrl) {
    if (imageUrl) {
        ...
        var imageSrc = imageUrl.substring(0,3)=='img' ? apiUrls.pictureUrl + imageUrl : imageUrl;
