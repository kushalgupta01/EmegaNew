...
document.addEventListener('DOMContentLoaded', async function() {
	...
	page.els.ordersSelect = document.getElementById('content-store-select');
...

	{
		...
		for (let form of forms) {
			...
			/*else if (form.type == 'suppliers') {
				...
				// Add entry for order type
				let collectType = ['All others', 'Hobbycos', 'Warehouse Collected', 'Partial Collected', 'Habitania', 'Autowell', 'Deal', 'HUGGIES', 'FINISH', 'ORALB', 'PACO', 'ICTONE', 'ATPACK'];
				let dataset = form.hasOwnProperty('filter') ? {filter: form.filter} : null;
...

	{
		...
		page.els.ordersSelect.addEventListener('click', getSelectedStoreOrders, false);
...
async function getOrders() {
	...
	do {
		let response, data;
		try {
			response = await fetch(apiServer+'orders/get/all');
		...
		// Add orders to table
		let cols = [
			'StoreID', 'DateCreated', 'RecordID', 'UserID', 'TotalPrice', 'BuyerFirstName', 'BuyerLastName', 'BuyerFullName',
			'BuyerAddress1', 'BuyerAddress2', 'BuyerCity', 'BuyerState',
			'BuyerPostcode', 'BuyerCountry', 'PhoneNumber', 'deliveryNote', 'totalSKUs', 'numberOfItems'
		];
		let colsNotEditable = {
			'StoreID': 1, 'DateCreated': 1, 'RecordID': 1, 'UserID': 1, 'TotalPrice': 1, 'BuyerFirstName': 1, 'BuyerLastName': 1, 'numberOfItems': 1, 'totalSKUs': 1
		};
		let colsSelectText = {
		...
		for (let storeID in data.orders) {
			if (storeID==81 || storeID==82) {
				for (let orderData of data.orders[storeID]) {
					if (!b2bOrders.hasOwnProperty(storeID)) {
						b2bOrders[storeID] = [];
					}

					b2bOrders[storeID].push(orderData);
					
					let order = orderData.data;
					let datecreate = order.DateCreated;
					if (!dates.includes(datecreate)) {
						dates.push(datecreate);
					}
				}
				continue;
			}
			for (let orderData of data.orders[storeID]) {
				...
				tr.dataset.createdDate = orderData.data.DateCreated.slice(0,10);
				...
				for (let col of cols) {
					...
					let text = '';
					let orderID = stores[storeID] ? stores[storeID].name : '';
					switch (col) {
						case 'StoreID':
							//text = stores[storeID].name;
							text = stores[storeID] ? stores[storeID].name : '';
							break;
						case 'deliveryNote':
				...
				let notPaid = false;
				let check = stores[storeID] ? stores[storeID].checkPayment : '';
				if (check !== false) {
		...
		for (let date of dates) {
			for (let storeID in b2bOrders) {
				for (let orderData of b2bOrders[storeID]) {
					let order = orderData.data;
					console.log(order);
					if (order.DateCreated == date) {
						// Add order details
						let tr = document.createElement('tr'), trMsg = null;
		...
		//let matches = tableBody.querySelectorAll("tr[data-store='81'],tr[data-store='82']");

		let trs = tableBody.querySelectorAll("tr");
		let totalOrders = {};
		for (let tr of trs){
			if (tr.dataset.notpaid == 'true' || tr.className.includes('msg2')==true){
				continue;
			}
			let store = tr.dataset.store;
			if (!totalOrders[store]) totalOrders[store] = 1
			else totalOrders[store]++	
		}
		let notice = document.querySelector('.notice');
		if (Object.keys(totalOrders).length > 0) {
			let li = document.createElement('li');

			//li.innerHTML = 'Total B2B orders: <b>'+matches.length+'</b>';
			for (let store in totalOrders){
				let strNm = stores[store] ? stores[store].name : '';
				//console.log(strNm+' Total Orders: '+valve[store]);
				li.innerHTML = li.innerHTML + strNm+' Total Orders: <b>'+totalOrders[store]+'</b><br>';
			}
...
async function getCollectedOrders() {
	...
	do {
		...
		let cols = [
			...
			'BuyerAddress1', 'BuyerAddress2', 'BuyerCity', 'BuyerState',
			'BuyerPostcode', 'BuyerCountry', 'PhoneNumber', 'Note'
		];
		...
		for (let storeID in data.orders) {
			for (let orderData of data.orders[storeID]) {
				...
				let isDeal = storeID==1 ? true : false;
				let isHuggies = storeID==1 ? true : false;
				let isFinish = storeID==1 ? true : false;
				let isOralb = storeID==1 ? true : false;
				let isPaco = storeID==1 ? true : false;
				let isIctone = storeID==1 ? true : false;
				if (storeID==1) {
					for (let item of orderData.data.Items) {
						if (!item.SKU || !item.SKU.startsWith('deal')) {
							isDeal = false;
						}
						if (!item.SKU || item.SKU != 'deal_HUGGIESBABYWIPES') {
							isHuggies = false;
						}
						if (!item.SKU || item.SKU != 'deal_FINISH_LEMON_110PACK') {
							isFinish = false;
						}
						if (!item.SKU || item.SKU != 'deal_ORALB_ELECTRIC_MEDIUM') {
							isOralb = false;
						}
						if (!item.SKU || !item.SKU.startsWith('PJ_')) {
							isPaco = false;
						}
						if (!item.SKU || !item.SKU.startsWith('ICT_')) {
							isIctone = false;
						}
					}
				}
				...
				tr.dataset.store = storeID;
				tr.dataset.deal = isDeal ? 1 : 0;
				tr.dataset.huggies = isHuggies ? 1 : 0;
				tr.dataset.finish = isFinish ? 1 : 0;
				tr.dataset.oralb = isOralb ? 1 : 0;
				tr.dataset.paco = isPaco ? 1 : 0;
				tr.dataset.ictone = isIctone ? 1 : 0;
				tr.dataset.id = orderData.id;
				tr.dataset.sku = orderData.data.Items[0].SKU;
				...
				if (collect=='All others') {
					if (SUPPLIER_INFO[SUPPLIER.HOBBYCO].stores.includes(storeID) || SUPPLIER_INFO[SUPPLIER.HABITANIA].stores.includes(storeID)
						|| SUPPLIER_INFO[SUPPLIER.AUTOWELL].stores.includes(storeID) || isDeal || isPaco || isIctone || SUPPLIER_INFO[SUPPLIER.ATPACK].stores.includes(storeID)) {
						tr.classList.add('hide');
					} else {
						tr.classList.remove('hide');
					}
				} else if (collect=='Hobbycos'){
					if (SUPPLIER_INFO[SUPPLIER.HOBBYCO].stores.includes(storeID)) {
				...
				} else if (collect=='Warehouse Collected'){
					if (SUPPLIER_INFO[SUPPLIER.HOBBYCO].stores.includes(storeID)) {
						if (orderData.status==19) {
							tr.classList.remove('hide');
						} else {
							tr.classList.add('hide');
						}
				...
				} else if (collect=='Partial Collected'){
					if (SUPPLIER_INFO[SUPPLIER.HOBBYCO].stores.includes(storeID)) {
				...
				} else if (collect=='Habitania'){
					if (SUPPLIER_INFO[SUPPLIER.HABITANIA].stores.includes(storeID)) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
				} else if (collect=='Autowell'){
					if (SUPPLIER_INFO[SUPPLIER.AUTOWELL].stores.includes(storeID)) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
				} else if (collect=='Deal'){
					if (isDeal) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
				} else if (collect=='HUGGIES'){
					if (isHuggies) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
				} else if (collect=='FINISH'){
					if (isFinish) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
				} else if (collect=='ORALB'){
					if (isOral) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
				} else if (collect=='PACO'){
					if (isPaco) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
				} else if (collect=='ICTONE'){
					if (isIctone) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
				} else if (collect=='ATPACK'){
					if (SUPPLIER_INFO[SUPPLIER.ATPACK].stores.includes(storeID)) {
						tr.classList.remove('hide');
					} else {
						tr.classList.add('hide');
					}
...
async function showCollected() {
	...
	for (let tr of trs) {
		...
		let isDeal = tr.dataset.deal=='1' ? true : false;
		let isHuggies = tr.dataset.huggies=='1' ? true : false;
		let isFinish = tr.dataset.finish=='1' ? true : false;
		let isOralb = tr.dataset.oralb=='1' ? true : false;
		let isPaco = tr.dataset.paco=='1' ? true : false;
		let isIctone = tr.dataset.ictone=='1' ? true : false;
...
async function addOrdersCollect() {
	...
	try {
		...
		let response = await fetch(apiServer+'orders/addcollect', {method: 'post', headers: {'DC-Access-Token': page.userToken}, body: formData});
		let data = await response.json();
...
async function addOrdersPrint() {
	...
	try {
		...
		let response = await fetch(apiServer+'orders/addprint', {method: 'post', headers: {'DC-Access-Token': page.userToken}, body: formData});
		let data = await response.json();
				...
				for (let col of cols) {
					...
					switch (col) {
						case 'id':
							...
							break;
						case 'StoreID':
							text = stores[storeID] && stores[storeID].name ? stores[storeID].name : '';
							break;
						case 'deliveryNote':
							text = orderData.deliveryNote;
							break;
...
function showPanel(data = {}) {
	...
	if (tabId == 'content-download') {
		...
		for (let input of page.els.downloadForm.querySelectorAll('input')) {
			...
			if (input.value==8) show = false;
			if (input.value==16) show = false;
			if (input.value==91) show = false;
			input.disabled = !show;
		...
		for (let input of page.els.downloadProductForm.querySelectorAll('input')) {
			let show = (input.dataset.service == SERVICE_IDS['EBAY'] || input.dataset.service == SERVICE_IDS['WOOCOMMERCE'] || input.dataset.service == SERVICE_IDS['SHOPIFY'] || input.dataset.service == SERVICE_IDS['NETO'] || input.dataset.service == SERVICE_IDS['CATCH']
				|| input.dataset.service == SERVICE_IDS['KOGAN'] || input.dataset.service == SERVICE_IDS['MAGENTO'] || input.dataset.service == SERVICE_IDS['BIGCOMMERCE'] || input.dataset.service == SERVICE_IDS['MULTISTORE'] || input.dataset.service == 'all');
			input.disabled = !show;
		...
		document.querySelector('#content-download-tracking #datefrom').value = '';
		document.querySelector('#content-download-tracking #dateto').value = '';
	}
...
async function downloadTracking() {
	...
	let datefrom = document.querySelector('#content-download-tracking #datefrom').value;
	let dateto = document.querySelector('#content-download-tracking #dateto').value;
	...
	try {
		...
		if (data.result == 'success') {
		    ...
		    if (store==71) head.push('SalesChannel');
		    aoa_data.push(head);
		    ...
		    for (let order of newOrders) {
		    	...
		    	for (let item of items) {
		    		...
		    		aoa_row.push(item.sku || item.SKU || item.offer_sku || item.SellerSku);
		    		aoa_row.push(item.title || item.name || item.ProductName || item.product_title || item.ProductTitle);
		    		aoa_row.push(item.quantity || item.Quantity);
		    		if (store != 30) {
			    		aoa_row.push(item.unitPrice || item.UnitPrice || item.price || item.price_unit);
			    		aoa_row.push(item.shippingPrice || (order.ShippingTotal ? order.ShippingTotal.replace(/"/g,"") : order.ShippingTotal ) || item.shipping_price);
		    		...
		    		if (store==71) aoa_row.push(order.SalesChannel ? order.SalesChannel.replace(/"/g,"") : order.SalesChannel);
...
async function getBigCommerceTracking() {
	...
	for (let tr of tableBodyTrs) {
		...
		if (trackingNo.startsWith('IZ') || trackingNo.startsWith('NZ') || trackingNo.startsWith('BN') || trackingNo.startsWith('QC') || trackingNo.startsWith('QX')  || trackingNo.startsWith('MP')) {
			carrier = 'fastway-au';
		} else if (trackingNo.startsWith('EMG')) {
			carrier = 'delivere';
		} else {
...
async function downloadQVBStockSentOrder() {
	...
	try {
		...
		if (data.result == 'success') {
			...
			var wb = XLSX.utils.book_new();
		    var aoa_data = [];
		    var head = ['Store', 'SalesRecordID', 'OrderID', 'LineItemID', 'SKU', 'QTY'];
		    	...
		    	for (let linitemid in locationselected) {
		    		...
		    		for (let loc of locations) {
		    			if (loc.id=='qvb') {
		    				let qty = loc.indivQty;
		    				let sku = loc.customSku;
		    				aoa_data.push([stores[order.store].name, order.salesRecordID, order.orderID, linitemid, sku, qty]);
...
async function downloadPackedOrders() {
	...
	try {
		...
		if (data.result == 'success') {
			 ...
			 var wb = XLSX.utils.book_new();
		     var aoa_data = [];
		     var head = ['OrderID', 'Name', 'Tracking', 'Status', 'SKU', 'Title', 'Scanned Qty', 'Total Qty', 'Missing Qty', 'Collector', 'Collected Time'];
		     aoa_data.push(head);
		     ...
		     for (let sr of packedOrders) {
		     	...
		     	for (let item of orderData.items) {
		     		...
		     		let date = new Date(sr.collected.replace(' ','T') + '.000Z');
		     		aoa_row.push(date.getFullYear() + '-' + (date.getMonth()+1) + '-' + date.getDate() + ' ' + date.getHours() + ':'+date.getMinutes()+':'+date.getSeconds());
		     		aoa_data.push(aoa_row);
		     	}
		     }
		     //console.log(aoa_data);
