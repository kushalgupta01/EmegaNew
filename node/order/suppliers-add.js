...
const addSupplierDetails = async function(req, res, next) {
	...
	try {
		do {
			...
			if (id != undefined && id != null && id != 'undefined') {
				...
				await conn.query ('UPDATE suppliers SET firstname = '+ conn.connection.escape(firstname)
				...
				await conn.query ('INSERT INTO suppliers (firstname, lastname, address1, address2, suburb, state, postcode, country, phone, email, store) VALUES (' + 
					conn.connection.escape(firstname) + ', ' + conn.connection.escape(lastname) + ', ' + conn.connection.escape(address1) + ', ' + conn.connection.escape(address2) + ', ' 
...
module.exports = addSupplierDetails;