...
const clientAddProduct = async function(req, res, next) {
	...
	try {
		do {
			...
			let sql = 'select 1 from stockinventory where store = ' + conn.connection.escape(store) + ' and '
				+ '(itemBarcode = ' + conn.connection.escape(itemBarcode) + ''
				+ ' or sku = ' + conn.connection.escape(sku) + ''
				+ ' or itemNo = ' + conn.connection.escape(itemNo) + ''
				+ ')';
			console.log(sql);
...
module.exports = clientAddProduct;