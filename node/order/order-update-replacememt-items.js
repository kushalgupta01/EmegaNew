...
const updateReplacementItems = async function(req, res, next) {
				...
				for (let lineItem of lineItems) {
					for(let item of items) {
						if (lineItem[itemConvert.LineItemID] == item.lineitemid) {
							let replacedItem = {};
							replacedItem.sku = item.replaceSku;
							replacedItem.name = item.replaceName;
							replacedItem.price = item.replacePrice;
							replacedItem.quantity = item.replaceQty;
							replacedItem.sent = 'true';
							if (storeID != 71) {
								replacedItem.itemID = item.replaceItemNo;
							...
							lineItem.replacedItem = replacedItem;
						}
					}
				}
			}
...
module.exports = updateReplacementItems;
