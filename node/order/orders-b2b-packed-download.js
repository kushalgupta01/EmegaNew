...
const downloadB2BPackedOrders = async function(req, res, next) {
	...
	do {
		...
		try{
			...
			let sql = '';
			if (datefrom && dateto) {
				sql = 'SELECT o.id, o.store, o.data, o.deliveryNote, o.salesRecordID, o.createdDate, c.trackingID, c.notes, c.status, c.locationNotes FROM orders o, collecting c WHERE (c.status = 21 or c.status = 22 or c.status = 3 or c.status = 23) AND c.orderID = o.id AND o.store = ' + conn.connection.escape(store) + ' AND (c.collected between "' + datefrom + '" AND "' + dateto + '")';
			} else {
				sql = 'SELECT o.id, o.store, o.data, o.deliveryNote, o.salesRecordID, o.createdDate, c.trackingID, c.notes, c.status, c.locationNotes FROM orders o, collecting c WHERE (c.status = 21 or c.status = 22 or c.status = 3 or c.status = 23) AND c.orderID = o.id AND o.store = ' + conn.connection.escape(store);
			}
...
module.exports = downloadB2BPackedOrders;