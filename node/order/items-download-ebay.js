...
const downloadItemsEbay = async function(req, res, next) {
	...
	do {
		...
		try{
			...
			var apiCall = 'GetSellerList';
			var totime = new Date();
			totime.setDate(totime.getDate()+60);
			var fromtime = new Date();
			var requestBody = '<?xml version="1.0" encoding="utf-8" ?>\
			<GetSellerListRequest xmlns="urn:ebay:apis:eBLBaseComponents">\
			<RequesterCredentials><eBayAuthToken>'+tokens[store]+'</eBayAuthToken></RequesterCredentials>\
			<Pagination><EntriesPerPage>2</EntriesPerPage><PageNumber>1</PageNumber></Pagination>\
			<IncludeVariations>true</IncludeVariations>\
			<GranularityLevel>Coarse</GranularityLevel>\
			<EndTimeFrom>'+fromtime.toISOString()+'</EndTimeFrom><EndTimeTo>'+totime.toISOString()+'</EndTimeTo>\
			...
			var responseJson = await new Promise((resolve, reject) => {
				parseString(responseXml, function (err, result) {
					//console.log(result.GetSellerListResponse.Errors);
			...
			const totalNumOfItems = responseJson.GetSellerListResponse.PaginationResult[0].TotalNumberOfEntries[0];
			...
			await (async function() {
				for (let j=1; j<=totalNumOfPages; j++){
					...
					<GetSellerListRequest xmlns="urn:ebay:apis:eBLBaseComponents">\
					<RequesterCredentials><eBayAuthToken>'+tokens[store]+'</eBayAuthToken></RequesterCredentials>\
					<Pagination><EntriesPerPage>200</EntriesPerPage><PageNumber>'+j+'</PageNumber></Pagination>\
					<IncludeVariations>true</IncludeVariations>\
					<GranularityLevel>Coarse</GranularityLevel>\
					<EndTimeFrom>'+fromtime.toISOString()+'</EndTimeFrom><EndTimeTo>'+totime.toISOString()+'</EndTimeTo>\
					</GetSellerListRequest>';
					...
					var items = responseJson.GetSellerListResponse.ItemArray[0].Item;
						...
						for (i=0; i<items.length; i++){
				          ...
				          if (result1.length == 0){
				          		if(!item.Variations) {
							            ...
							            var qtyAvailable = conn.connection.escape(item.Quantity[0]);

							            if (item.SKU) {
							            	sku = conn.connection.escape(item.SKU[0]);
							            }

							            var price = conn.connection.escape(item.SellingStatus[0].CurrentPrice[0]._);

							            var sql2 = 'INSERT IGNORE INTO items (itemNo, itemStore, itemName, itemPhoto, sku, customSku, ebayquantity, itemPrice) VALUES ('+ itemId +','+store+','+ title +','+ picUrl +','+ sku+ ','+ sku+','+ qtyAvailable+','+ price+')';
					                    ...
					                    console.log("Item " + itemId + " inserted");
						          		...
						          		for (let k=0; k<variations.length; k++) {
						          			...
						          			var qtyAvailable = conn.connection.escape(variation.Quantity[0]);
						          			var specs = variation.VariationSpecifics[0].NameValueList;
						          			var variationSuffix = '[' + specs.map(spec => spec.Value).join(',') + ']';
						          			
						          			var title = conn.connection.escape(item.Title[0] + variationSuffix);
						          			var sku = conn.connection.escape(null);
								            if (variation.SKU) {
								            	sku = conn.connection.escape(variation.SKU[0]);
								            }

								            var price = conn.connection.escape(variation.StartPrice[0]._);

								            var sql3 = 'INSERT IGNORE INTO items (itemNo, itemStore, itemName, itemPhoto, sku, customSku, ebayquantity, itemPrice) VALUES ('+ itemId +',' + store + ','+ title +','+ picUrl +','+ sku+','+ sku+ ','+ qtyAvailable+','+ price+')';
							                //var sql = 'UPDATE items set sku = ' + sku + ' where itemNo = ' + itemId;
				              ...
				              if(!item.Variations) {
							            ...
							            var sql2 = 'UPDATE items SET itemName = ' + title + ', sku = ' + sku + ', customSku = ' + sku + ', itemPhoto=' + picUrl + ', ebayquantity='+ qtyAvailable +', itemPrice='+ price +' WHERE itemNo = ' + itemId;
						                //var sql = 'UPDATE items set sku = ' + sku + ' where itemNo = ' + itemId;
				              ...
				              if(item.Variations) {
				              	let variations = item.Variations[0].Variation;
				              	  for (let variation of variations) {
				              	  		...
				              	  		var specs = variation.VariationSpecifics[0].NameValueList;
						          		var variationSuffix = '[' + specs.map(spec => spec.Value).join(',') + ']';
				              	  		let title = conn.connection.escape(item.Title[0] + variationSuffix);
							            ...
							            if (variation.SKU) {
								            ...
								            var result2 = await conn.query(sql2);
								            if (result2.length>0) {
								            	var sql3 = 'UPDATE items SET itemName = ' + title + ', itemPhoto=' + picUrl + ', ebayquantity='+ qtyAvailable+ ', customSku = ' + sku +', itemPrice = '+ price  +' WHERE sku = ' + sku + ' and itemNo = ' + itemId;
								                //var sql = 'UPDATE items set sku = ' + sku + ' where itemNo = ' + itemId;
