...
const downloadDispatched = async function(req, res, next) {
		...
		try{
			...
			if (datefrom && dateto) {
				if (store==30) {
					...
					+ 'and (o.store = 31 or o.store = 32 or o.store = 33 or o.store = 35) ' + ' AND (c.packedTime between "' + datefrom + '" AND "' + dateto + '") ORDER BY salesRecordID, store ' ;
					...
					+ ' c.trackingID, c.type, o.createdDate, c.packedTime, c.status, c.packedData, c.collected, c.notes, c.weight FROM orders o, collecting c WHERE o.id = c.orderID ' 
					+ 'and o.store = ' + store + ' AND (c.packedTime between "' + datefrom + '" AND "' + dateto + '") ORDER BY salesRecordID' ;
				} else if (Config.STORES[store].service == Config.SERVICE_IDS.EBAY) {
					...
					+ store + ' AND (c.packedTime between "' + datefrom + '" AND "' + dateto + '") ORDER BY name' ;
...
module.exports = downloadDispatched;