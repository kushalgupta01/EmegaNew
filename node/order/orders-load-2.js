	...
	do {
		...
		if (orderStatus !== null) {
			...
			for (let item of orderStatus) {
				sqlWhereItems.push('status = '+conn.connection.escape(item));
			}
			if (partialrefund=='true') sqlWhereItems.push('collecting.partialrefund = 1');
			sqlWhere = '('+sqlWhereItems.join(' OR ')+') AND ';
		}

		let sql = 'SELECT collecting.*, orders.store, orders.orderID AS ordersOrderID FROM collecting, orders WHERE '+sqlWhere+'collecting.orderID = orders.id';
		if (morelabel=='true') sql = sql + ' AND collecting.morelabel > 0';
		if (supplier==13) sql = sql + ' AND (orders.store = 102 OR (orders.store = 1 AND LOWER(JSON_EXTRACT(JSON_EXTRACT(orders.data,"$.items"),"$[*].sku")) like "%rrv_%"))';
		...
		try {
			...
			let sql = 'SELECT o.* FROM orders o inner join collecting c on o.id = c.orderID WHERE '+(dateList.start ? 'o.addedDate '+(dateList.end ? '>=' : '=')+' '+conn.connection.escape(dateList.start)+' AND ' : '')+(dateList.end ? 'o.addedDate <= '+conn.connection.escape(dateList.end)+' AND ' : '')+'o.cancelled = 0'
