...
const getNextRecord = async function (req, res, next) {
    ...
    try {
        do {
            ...
            var sql1 = 'SELECT * FROM collecting c, orders o WHERE c.status = 2 AND c.type = '
                    + typeMS + ' AND c.packer = ' + usernameMS + ' AND o.store = '+store +' AND c.orderID = o.id LIMIT 1';
            var sql2 = 'SELECT * FROM collecting c, orders o WHERE c.status = 10 AND c.type = '
                    + typeMS + ' AND c.packer = ' + usernameMS + ' AND o.store = '+store +' AND c.orderID = o.id LIMIT 1';
            var sql3 = 'SELECT * FROM collecting c, orders o WHERE c.status = 9 AND c.type = '
                    + typeMS + ' AND c.packer = ' + usernameMS + ' AND o.store = '+store +' AND c.orderID = o.id LIMIT 1';
            ...
            if (sql1result && typeof sql1result != 'undefined' &&
                sql1result.length > 0) {
                record = sql1result;
            } else if (sql2result && typeof sql2result != 'undefined' &&
                sql2result.length > 0) {
                record = sql2result;
            } else if (sql3result && typeof sql3result != 'undefined' &&
                sql3result.length > 0) {
                record = sql3result;
            } else {
                httpStatus = 404;
