...
const brandSku = async function(req, res, next) {
	...
	try {
		do {
			...
			if(req.query.sku){
				...
				if (method == 'get') {
					brandDetails = await conn.query('SELECT s.brand FROM stockinventory s where s.sku ='+conn.connection.escape(sku));
					if(brandDetails != null){
						output.result = "success";
						httpStatus = 200;						
						output.brandData = {
							brandName : brandDetails[0].brand
						}; 
						}else{
