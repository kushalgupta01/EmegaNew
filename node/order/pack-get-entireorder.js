...
const getEntireOrder = async function (req, res, next) {
    //console.log(req.query.recordData);
    ...
    try {
        do {
            ...
            for (var i = 0; i < entireOrder.length; i++) {
                ...
                var packedTime = null;
                var intervalBeforeNow = null;
                if (entireOrder[i].packedTime) {
                    packedTime = dateSqlToJsDate(entireOrder[i].packedTime);
                    intervalBeforeNow = new Date();
                    intervalBeforeNow.setMinutes(intervalBeforeNow.getMinutes()
                        - Config.PACKING.TIMEOUT);
                }
                if (packedTime != null && intervalBeforeNow != null) {
                    if ((packedTime >= intervalBeforeNow
                        && entireOrder[i].packer != user.username
                        && entireOrder[i].status == 2)
                        || entireOrder[i].status == 3) {
                        check = true;
                    }
                }
            }
            ...
            if (sqlArray.length > 0) {
                sqlUpdate = "UPDATE collecting SET status = 2, packer = " +
                    usernameMS + ", packedTime = UTC_TIMESTAMP() WHERE (" +
                    sqlArray.join(" OR ") + ")";
            }
