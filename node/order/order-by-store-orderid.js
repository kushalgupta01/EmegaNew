...
const orderByStoreOrderid = async function(req, res, next) {
	...
	try {
		do {
			...
			if(req.query.store && req.query.orderId){
				...
				if (method == 'get') {
					orderData = await conn.query('SELECT o.id, o.data -> "$.SalesRecordID" recordId, o.data -> "$.OrderID" orderId, o.data -> "$.SalesChannel" channels,o.data -> "$.DateInvoiced" purchaseDate, c.type, c.status, c.trackingID FROM orders o, collecting c WHERE o.store = '+storeVal+' AND o.orderID = '+ conn.connection.escape(orderId)+' LIMIT '+totalRecords+' OFFSET '+offset);
					if(orderData != null){
...
module.exports = orderByStoreOrderid;