...
const inventoryDetailsByStoreSku = async function(req, res, next) {
	...
	try {
		do {
			...
			if(req.query.store && req.query.sku){
				let storeVal = null;
				sku = req.query.sku;
				...
				if (method == 'get') {
					invertoryDetails = await conn.query('SELECT s.itemName, s.sku, s.itemBarcode, s.indivQty, s.weight FROM stockinventory s WHERE s.store = '+storeVal+' AND s.sku = '+sku);
					if(invertoryDetails != null){
						output.result = "success";
						httpStatus = 200;
						let invData = {};
						for(let data of invertoryDetails)						
							invData = { sku : data.sku, itemName : data.itemName, qty : data.indivQty, barcode : data.itemBarcode, weight : data.weight};
				...
				output.result = 'Pass the store/sku value';
				output.status = 404;
				httpStatus = 404;
			}
...
module.exports = inventoryDetailsByStoreSku;