const Database = require('./connection');
const {Config} = require('./config');
const {userCheckToken} = require('./users-token');

const loadPageItemIDbyBarcode = async function(req, res, next) {
	var conn = new Database(dbconn);
	var method = req.method.toLowerCase();
	var searchValue = req.query.searchvalue || ''
	var searchField = req.query.searchfield || null

	var token = req.header(Config.ACCESS_TOKEN_HEADER);
	// var store = req.params.store;
	var output = {result: null};res
	var httpStatus = 400;
	// console.log(searchField);
	// console.log('searchvalue '+searchValue);
	try {
		do {
			// Check token
			if (!await userCheckToken(token)) {
				httpStatus = 401;
				output.result = 'Not logged in.';
				break;
			}

			var sql = 'SELECT * FROM stockinventory WHERE ';
			if(searchField != null && searchField != '') {
				if(searchField == 'itemno') {
					sql = sql + "itemNo ='" + searchValue+"'";
				}

				if(searchField == 'itemname') {
					sql = sql + 'itemName like "%' + searchValue + '%"';
				}

				if(searchField == 'sku') {
					sql = sql + "sku ='" + searchValue+"'";
				}

				if(searchField == 'customsku') {
					sql = sql + "customSku ='" + searchValue+"'";
				}

				if(searchField == 'itembarcode') {
					sql = sql + "itemBarcode ='" + searchValue+"'";
				}
			}
			await conn.connect();
			var id = await conn.query(sql);
			//console.log(id);
			if (id.length==0) {
				httpStatus = 404;
				output.result = 'Not bay found.';
				break;
			}

			// var stockData = {};
			// for (let stock of stocks) {
				// stock['locations'] = await conn.query('SELECT * FROM inventorylocation WHERE invID=' + stock.id);
				// stockData[stock.id] = stock;		
			output.invID = id[0].id;
			output.ids = id;
			output.result = 'success';
			httpStatus = 200;
		} while(0);
	}
	catch (e) {
		// Error
		httpStatus = 503;
		output.result = e;
		console.log(e);
	}
	if (conn.connected) conn.release();

	res.set({
		'Cache-Control': 'no-store, no-cache, must-revalidate, max-age=0, post-check=0, pre-check=0',
		'Pragma': 'no-cache',
	});
	res.json(httpStatus, output);
	next();
}

module.exports = loadPageItemIDbyBarcode;