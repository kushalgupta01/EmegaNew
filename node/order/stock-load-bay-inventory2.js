...
const loadStockBayInventory = async function(req, res, next) {
	...
	try {
		do {
			...
			if (scanned) {
				sql = 'SELECT i.*, s.store, s.sku, s.itemName, s.quantityPerCarton, s.image FROM inventorylocation i, stockinventory s WHERE i.invID = s.id  AND i.bay = ' + conn.connection.escape(scanned);			
			} 
